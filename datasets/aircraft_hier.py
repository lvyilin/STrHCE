...
class Aircraft(VisionDataset):
    ...
    def __init__(self, root, train=True, class_type='variant', transform=None,
        ...
        (_, family_targets, _, _) = self.find_family_classes()
        samples = self.make_dataset(image_ids, targets, family_targets)
    ...
    def find_family_classes(self):
        image_ids = []
        targets = []
        with open(self.family_classes_file, 'r') as f:
            for line in f:
                split_line = line.split(' ')
                image_ids.append(split_line[0])
                targets.append(' '.join(split_line[1:]))
    ...
    def make_dataset(self, image_ids, targets, family_targets):
        assert (len(image_ids) == len(targets))
        images = []
        for i in range(len(image_ids)):
            item = (os.path.join(self.root, self.img_folder,
                                 '%s.jpg' % image_ids[i]), targets[i], family_targets[i])
            images.append(item)
        return images
